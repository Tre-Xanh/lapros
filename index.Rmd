...
Nếu $\Lmtx\neq\emptyset$ thì ta có thể chọn một nhãn:
\begin{equation}
\lmtx \defined
\begin{cases}
\mathop{\rm arg max} \limits_{l\in \Lmtx} \pc_\model(\yo=l;\x), & \text{nhãn có xác suất dự đoán lớn nhất}\\
\mathop{\rm arg max} \limits_{l\in \Lmtx} \{\pc_\model(\yo=l;\x) - \thres_l\}, & \text{nhãn có xác suất trội nhất so với chỉ tiêu}
\end{cases}
(\#eq:lmtx)
\end{equation}
